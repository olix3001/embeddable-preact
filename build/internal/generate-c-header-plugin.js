...
import ejs from 'ejs';
const asyncGzip = (data, options) => new Promise((resolve, reject) => gzip(data, options, (err, result) => {
    if (err)
        reject(err);
    else
        resolve(result);
...
const viteGenerateCHeader = ({ bundlePreact = false } = {}) => {
    ...
    return {
        ...
        async closeBundle() {
            // Load manifest.
            const siteManifest = JSON.parse(readFileSync(join(config.build.outDir, 'site-manifest.json'), 'utf-8'));
